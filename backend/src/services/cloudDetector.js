...
function inputUrlFromCamera(cam) {
  ...
  if (cam.streamType === "RTSP" && cam.ip) {
    ...
    log.info(
      { camera: cam.name, url: url.replace(/:([^:@]+)@/, ":****@") },
      "Built RTSP URL"
    );
    return url;
  }
  ...
  const base =
    cam.webrtcBase?.replace(/:\d+$/, ":8888") || "http://127.0.0.1:8888";
  ...
  (async function loop() {
    while (alive) {
      try {
        ...
        if (fire) {
          log.warn(
            { cam: cam.name, userId: cam.userId },
            "FIRE DETECTED - Lambda will trigger SNS"
          );
        } else {
          log.info({ cam: cam.name, fire }, "No fire detected");
        }
      } catch (e) {
        log.warn({ cam: cam.name, err: String(e) }, "Cloud detection failed");
      }

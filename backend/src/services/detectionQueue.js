...
//         if (
//           state.consecutiveDetections >= CONSECUTIVE_DETECTIONS_REQUIRED &&
//           !state.isFire &&
...
//         if (
//           state.consecutiveClear >= CONSECUTIVE_CLEAR_REQUIRED &&
//           state.isFire
//         ) {
//           state.isFire = false;
...
// Track detection state per camera
const cameraStates = new Map(); // cameraId -> { isFire, lastChecked }
...
async function startQueueLoop() {
  ...
  async function loop() {
    ...
    try {
      ...
      // ✅ SIMPLE LOGIC:
      // Fire detected (result.isFire = true) → Broadcast START
      // No fire detected (result.isFire = false) → Do nothing
      if (result.isFire) {
        log.warn(
          {
            cameraId: camera.id,
        ...
        // ✅ Broadcast EVERY TIME fire is detected (even if already broadcasting)
        // This ensures stream starts again if user closed it
        log.warn(
          {
            cameraId: camera.id,
            name: camera.name,
        ...
        // ✅ No fire detected - just update state, don't broadcast anything
        log.info(
          {
            cameraId: camera.id,
            name: camera.name,

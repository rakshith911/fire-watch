...
async function listCameras(token) {
  ...
  console.log('\nYour Cameras:');
  cameras.forEach(cam => {
    console.log(`  ${cam.id}. ${cam.camera} - ${cam.location}`);
  });
  console.log('');
...
async function addCamera(userId) {
  ...
  const ip = await question('Camera IP (e.g., 192.168.1.201:554): ');
...
async function startDetection(token) {
  ...
  const choice = await question('Start detection for:\n  1. All cameras\n  2. Select specific cameras\nChoice: ');
  ...
  if (choice === '1') {
    ...
    const input = await question('Enter camera IDs (comma-separated, e.g., 1,3,5): ');
    cameraIds = input.split(',').map(id => parseInt(id.trim())).filter(id => !isNaN(id));
  ...
  try {
    const result = await apiCall(token, 'POST', '/api/cameras/start-detection', { cameraIds });
    
    console.log(`\n${result.message}`);
    
    if (result.started.length > 0) {
      console.log('\nStarted:');
      result.started.forEach(cam => console.log(`  - ${cam.name} (ID: ${cam.id})`));
    }
    
    if (result.failed && result.failed.length > 0) {
      console.log('\nFailed:');
      result.failed.forEach(cam => console.log(`  - ${cam.name} (ID: ${cam.id}): ${cam.error}`));
    }
...
async function stopDetection(token) {
  ...
  const choice = await question('Stop detection for:\n  1. All cameras\n  2. Select specific cameras\nChoice: ');
  ...
  try {
    const result = await apiCall(token, 'POST', '/api/cameras/stop-detection', { cameraIds });
    
    console.log(`\n${result.message}`);
    
    if (result.stopped.length > 0) {
      console.log('\nStopped:');
      result.stopped.forEach(cam => console.log(`  - ${cam.name} (ID: ${cam.id})`));
    }

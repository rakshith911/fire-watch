...
export default function AddCameraDialog({ onClose }) {
  ...
  const [form, setForm] = useState({
    name: "", location: "", ip: "", username: "", password: "",
    detection: "local",
    streamType: "hls",
    hlsUrl: "",
    webrtcGateway: "http://127.0.0.1:8889",
    webrtcName: "camX"
  });
  ...
  function submit(e) {
    ...
    const stream =
      form.streamType === "webrtc"
        ? { type: "webrtc", gatewayBase: form.webrtcGateway, name: form.webrtcName }
    ...
    <div className="modal">
      <div className="modal-card">
        ...
        <form onSubmit={submit} className="form">
          ...
          <div className="row">
            <label>IP</label>
            <input value={form.ip} onChange={e=>onChange("ip", e.target.value)} />
          </div>
          ...
          <div className="row">
            ...
            <select value={form.detection} onChange={e=>onChange("detection", e.target.value)}>
          ...
          <div className="row">
            ...
            <select value={form.streamType} onChange={e=>onChange("streamType", e.target.value)}>
          ...
          {form.streamType === "hls" && (
            <div className="row">
              <label>HLS URL</label>
              <input placeholder="http://.../cam1.m3u8" value={form.hlsUrl} onChange={e=>onChange("hlsUrl", e.target.value)} />
            </div>
          )}
          ...
          {form.streamType === "webrtc" && (
            <>
              <div className="row">
                ...
                <input value={form.webrtcGateway} onChange={e=>onChange("webrtcGateway", e.target.value)} />
              </div>
              <div className="row">
                <label>Path/Name</label>
                <input value={form.webrtcName} onChange={e=>onChange("webrtcName", e.target.value)} />
              </div>
            </>
          )}
